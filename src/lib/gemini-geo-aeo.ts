...
async function getCached(domain: string): Promise<LlmGeoAeoResult | null> {
  try {
    ...
    return {
      ...
      platformVisibility: data.platformVisibility ?? { chatGpt: data.geoLlmScore, gemini: data.geoLlmScore, perplexity: Math.max(0, data.geoLlmScore - 10), claude: Math.max(0, data.geoLlmScore - 5) },
      sentiment: data.sentiment ?? 'Positive',
      semanticGaps: data.semanticGaps ?? [],
